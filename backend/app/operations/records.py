...
def update_skater_records(gameType: int):
    with open(os.path.join(ROOT_PATH, 'sql', 'skater_season_records.sql'), 'r') as f:
        skater_query = f.read()
...
def update_goalie_records(gameType: int):
    with open(os.path.join(ROOT_PATH, 'sql', 'goalie_season_records.sql'), 'r') as f:
        goalie_query = f.read()
...
def update_max_games():
    with open(os.path.join(ROOT_PATH, 'sql', 'max_games.sql'), 'r') as f:
        query = f.read()
